...
This contract is complemented by the `StreamPort` interface (defined in `controls-composite-model`), which is designed for **continuous, low-latency** data flows and is built upon `kotlinx-io` for maximum performance.
