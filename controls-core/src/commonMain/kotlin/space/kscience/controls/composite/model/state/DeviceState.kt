...
public interface DeviceState<out T> {
    /**
     * The full state representation including a nullable value, timestamps, and quality.
...
/** The origin timestamp of the last state update. Accessing this is a shortcut for `stateValue.originTime`. */
public val <T> DeviceState<T>.timestamp: Instant get() = stateValue.originTime

/** The quality of the current state value. Accessing this is a shortcut for `stateValue.quality`. */
public val <T> DeviceState<T>.quality: DataQuality get() = stateValue.quality
...
public interface MutableDeviceState<T> : DeviceState<T> {
    /**
     * Updates the state with a new value, automatically setting origin and server timestamps to now
     * and quality to [DataQuality.GOOD].
     * @param value The new value to set.
     */
    public suspend fun update(value: T)

    /**
     * Updates the entire state with a new [StateValue], allowing for explicit control over value, timestamps, and quality.
     * This is useful for scenarios like propagating an error state or a null value from a connector.
     * @param stateValue The new full state value.
     */
    public suspend fun updateState(stateValue: StateValue<T?>)

    /**
     * A convenience function to update only the quality of the state, preserving the current value and timestamps.
     * Useful for marking a value as stale or invalid without changing it.
     * @param quality The new quality to set.
     */
    public suspend fun updateQuality(quality: DataQuality)
}
